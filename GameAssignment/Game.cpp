...
Game::Game() {
    ...
    windowScreenHeight = window->getHeight();
    windowScreenWidth = window->getWidth();
}
...
int gorkWidth = gamecharacter->getgorkWidth();
int gorkHeight = gamecharacter->getgorkHeight(); 
int militiaWidth = gamecharacter->getmilitiaWidth();
int militiaHeight = gamecharacter->getmilitiaHeight();
...
void Game::render(HRESULT hr, LPD3DXSPRITE sprite) {
    ...
    hr = D3DXCreateTextureFromFile(d3dDevice, "backgroundimage.png", &backgroundTexture); // Adjust the path to the correct location
    if (FAILED(hr)) {
...
void Game::update() {
    ...
    if (d3dDevice) {
        ...
        object->Update(platformPositions, platformBoundingBoxes, ladderBoundingBox, ladderBoundingBoxRight);
...
void Game::Logic(float deltaTime, AudioManager& audioManager) {
...
void Game::updateBoundingBox(D3DXVECTOR3 platformPositions[4], RECT platformBoundingBoxes[4], RECT ladderBoundingBox, RECT ladderBoundingBoxRight) {
    ...
    for (int i = 0; i < 4; i++) {
        platformBoundingBoxes[i].left = (LONG)platformPositions[i].x + buffer;
        platformBoundingBoxes[i].top = (LONG)platformPositions[i].y + buffer;
